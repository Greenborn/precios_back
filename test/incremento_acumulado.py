...
if incrementos_acumulados_por_dia:
    ...
    medias = [np.mean(incrementos_acumulados_por_dia[d]) for d in dias_ordenados]
    medianas = [np.median(incrementos_acumulados_por_dia[d]) for d in dias_ordenados]
    # Media diaria
...
if incrementos_acumulados_por_mes:
    ...
    medias = [np.mean(incrementos_acumulados_por_mes[m]) for m in meses_ordenados]
    medianas = [np.median(incrementos_acumulados_por_mes[m]) for m in meses_ordenados]
    # Media mensual
    plt.figure(figsize=(18, 8))
